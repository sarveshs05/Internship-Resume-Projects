...
class CAPM:
    ...
    def initialize(self):
        ...
        # print(stock_data)
        #considering monthly returns instead of daily returns
        stock_data = stock_data.resample('M').last()
        # print(stock_data)
        self.data = pd.DataFrame({'s_adjclose':stock_data[self.stocks[0]], 'm_adjclose':stock_data[self.stocks[1]]})
        #IBM is a stock, GSPC is a market and hence s and m
    ...
    def calculate_beta(self):
        ...
        print('Beta from calculation :', beta)

    def regression(self):
        beta, alpha = np.polyfit(self.data['m_returns'], self.data['s_returns'],deg=1)
        print('Beta from regression :', beta)
        expected_return = risk_free_rate + beta*(self.data['m_returns'].mean()*12-risk_free_rate)
        print('Expected Return: ', expected_return)
        self.plot_regression(alpha,beta)

    def plot_regression(self, alpha, beta):
        fig, axis = plt.subplots(1, figsize=(20, 10))
        plt.scatter(self.data["m_returns"], self.data['s_returns'],
                     label="Data Points")
